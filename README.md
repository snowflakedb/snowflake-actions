...
| You want… | Use | Docs |
|-----------|-----|------|
| The Snowflake CLI (default), optionally plus Cortex Code | `snowflakedb/snowflake-actions@v3` | this page |
| Only the Snowflake CLI, explicitly | `snowflakedb/snowflake-actions/snowflake-cli@v3` | [Snowflake CLI action](snowflake-cli/README.md) |
| Only the Cortex Code CLI | `snowflakedb/snowflake-actions/cortex-code@v3` | [Cortex Code CLI action](cortex-code/README.md) |
...
- `cortex-*` and `connection-name` apply only when `cortex-code: true`. **The standalone [`cortex-code`](cortex-code/README.md) action names these `cli-channel` / `cli-version`** — unknown `with:` keys are silently ignored by GitHub Actions, so use the right name for your entry point.
